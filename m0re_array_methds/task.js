    ...
    searchResults.forEach((ele) => {
        productListEl.innerHTML += `
                <div style="border: 1px solid black;">
                    <a href="" id="btn"><h3>${ele.name}</h3></a>
                    <p>Category: ${ele.category}</p>
                    <p>Price: ${ele.price}</p>
                    <p>Seller: ${ele.seller}</p>
                    <p>Description: ${ele.description}</p><br>
...
const clickButtonEl = document.getElementById("btn")

const clickedItemsEl = document.getElementById("clickedItems")

function clickProducts() {
    searchResults = products.filter((ele) => ele.name.toLowerCase().includes(searchQuery.toLowerCase()) || ele.category.toLowerCase().includes(searchQuery.toLowerCase()) || ele.seller.toLowerCase().includes(searchQuery.toLowerCase()) || ele.description.toLowerCase().includes(searchQuery.toLowerCase()))

    const clickResults = searchResults.find((ele) => ele.name)
    console.log(clickResults);

    clickResults.forEach((ele) => {
        clickedItemsEl.innerHTML += `
                    <p>Name: ${ele.name}</p>
                    <p>Category: ${ele.category}</p>
                    <p>Price: ${ele.price}</p>
                    <p>Seller: ${ele.seller}</p>
...
console.log(clickButtonEl);

clickButtonEl.addEventListener("click", clickProducts)